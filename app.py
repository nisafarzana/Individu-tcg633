...
# --------------------------------------------------------------------------------------
# Theme (Light is the default / what was requested; Dark kept available via the sidebar
# toggle for the original video-style look). Pure CSS injected into this single file -
# the look does not depend on any .streamlit/config.toml being present.
# --------------------------------------------------------------------------------------
DARK_CSS = """
<style>
...
section[data-testid="stSidebar"] { background-color: #f6f7f9; }
h1,h2,h3,h4,h5,h6, p, span, label, .stMarkdown { color: #1a1f29 !important; }
div[data-testid="stMetricValue"] { color: #1a1f29; }
hr { border-color: #e3e6ec; }
...
.pms-title { font-size: 2.1rem; font-weight: 800; margin-bottom: 0px; }
.pms-subtitle { color: #6b7280; font-size: 0.95rem; margin-top: -6px; margin-bottom: 22px; }
.pms-bignum { font-size: 2.6rem; font-weight: 800; line-height: 1.1; }
.pms-label { color: #6b7280; font-size: 0.85rem; text-transform: uppercase; letter-spacing: .04em; }
.pms-banner {
  border-radius: 8px; padding: 12px 16px; font-size: 0.98rem; margin-top: 10px;
  background: #eaf2ff; border: 1px solid #bcd6fb; color: #14385f;
}
.pms-gauge-wrap { margin-top: 18px; }
.pms-gauge-track {
  position: relative; height: 10px; border-radius: 6px; margin-top: 10px;
...
.pms-gauge-dot {
  position: absolute; top: -5px; width: 20px; height: 20px; border-radius: 50%;
  background: #ffffff; border: 4px solid var(--dotcolor, #C62828);
  transform: translateX(-50%); box-shadow: 0 0 4px rgba(0,0,0,.25);
}
.pms-pill {
  display:inline-block; padding: 2px 10px; border-radius: 999px; font-size: 0.78rem;
...
/* Native widget light styling */
button[data-testid^="stBaseButton-secondary"] {
  background-color: #ffffff !important; color: #1a1f29 !important;
  border: 1px solid #d8dce3 !important;
}
button[data-testid^="stBaseButton-secondary"]:hover {
  border-color: #2E75B6 !important; color: #2E75B6 !important;
}
input[data-testid="stTextInputRootElement"], div[data-testid="stTextInputRootElement"],
.stTextInput input, .stNumberInput input, div[data-testid="stNumberInputContainer"] {
  background-color: #ffffff !important; color: #1a1f29 !important;
...
div[data-baseweb="select"] > div {
  background-color: #ffffff !important; border-color: #d8dce3 !important; color: #1a1f29 !important;
}
div[data-baseweb="popover"] li { background-color: #ffffff !important; color: #1a1f29 !important; }
section[data-testid="stFileUploaderDropzone"] {
  background-color: #f6f7f9 !important; border: 1px dashed #d8dce3 !important;
}
section[data-testid="stFileUploaderDropzone"] button {
  background-color: #ffffff !important; color: #1a1f29 !important; border: 1px solid #d8dce3 !important;
}
div[data-testid="stDataFrame"] { border: 1px solid #e3e6ec !important; border-radius: 8px; }
</style>
"""
